let currentProfileIndex = 0;
        ...
        // ///////responsiveness close tab
  // Wait for the DOM to fully load
  document.addEventListener('DOMContentLoaded', function () {
    const smNavItems = document.querySelectorAll('.sm-nav');
    const offcanvasElement = document.getElementById('offcanvasTop');
    const offcanvasInstance = bootstrap.Offcanvas.getOrCreateInstance(offcanvasElement); // Bootstrap 5

    smNavItems.forEach(item => {
      item.addEventListener('click', () => {
        offcanvasInstance.hide(); // Closes the offcanvas
      });
    });
  });
